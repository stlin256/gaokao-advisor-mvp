...
本项目是一个H5单页应用，旨在为面临高考志愿填报最后抉择的考生及家长，提供一份由AI驱动的、结合了最新招生计划的深度分析报告。应用通过简洁的界面收集用户的基本信息和困惑，调用大语言模型进行智能分析，并以流式的效果实时展示结果，最后支持将报告保存为PDF。
...
项目使用 `_data/enrollment_data_2025.json` 文件作为预置的招生计划数据源。该文件为AI分析提供了核心的动态数据。
...
- **省份名称**: 例如 `"广东"`, `"北京"`。
- **科类名称**: 例如 `"物理类"`, `"历史类"`, `"不分科类"`。
- **院校名称**: 例如 `"中山大学"`。
- **专业名称**: 例如 `"计算机科学与技术"`。
- **change**: 字符串类型。表示招生人数的变化。`"+5"`表示扩招5人，`"-3"`表示缩招3人，`"0"`表示无变化。
- **notes**: 字符串类型。用于记录变动原因或其他备注，如`"扩招"`, `"新增专业方向"`。
...
"广东": {
  "物理类": {
    ...
    "华南理工大学": {
      "人工智能": { "change": "-3", "notes": "缩招" }
    }
  },
  "历史类": {
    "暨南大学": {
      "新闻学": { "change": "+10", "notes": "新增专业方向" }
    }
  }
}
```

在实际应用中，您需要根据最新的招生信息，维护和更新这个JSON文件。
---
