    ...
    def do_POST(self):
        ...
        try:
            ...
            if count >= DAILY_LIMIT:
                ...
                self.end_headers()
                self.wfile.write(json.dumps({"error": "非常抱歉，今日的免费体验名额已被抢完！请您明日再来。"}).encode())
                return
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"error": "服务暂时不可用，无法连接到数据库。"}).encode())
            return
        ...
        try:
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"error": "请求格式错误。"}).encode())
            return

        # 3. Prepare Prompt
        try:
            # Load enrollment data to enrich the prompt
            ...
            prompt = self.prepare_prompt(user_data, enrollment_data)
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"error": "准备分析数据时出错。"}).encode())
            return
        ...
        try:
            ...
            if not api_key or not base_url:
                raise ValueError("OPENAI_API_KEY or OPENAI_API_BASE not found in environment variables. Please set them in Vercel.")
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"error": "AI服务暂时无法响应，请稍后重试。"}).encode())
            return
        ...
        try:
            ...
            # Log the error but don't fail the request, as the user has already received the report
            print(f"KV increment failed: {e}")

        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        self.wfile.write(json.dumps({"report": report_markdown}).encode())

    def prepare_prompt(self, user_data, enrollment_data):
